...
We show significant improvements over prior models and 
...
We investigate three hypothesis for each feature which can distinguish PD and control


\begin{enumerate}
	\item The feature is likely inaudible to the human ear. The distributions of individuals without PD (control) and \textit{no speech difficulties} (NSD)\footnote{In this section, \emph{NSD} denotes the set of participants with Parkinson's disease and no speech difficulties and \emph{PD} denotes the set of participants with Parkinson's disease and speech difficulties.} being different would support this.
	\item The feature is likely audible to the human ear. This is supported by the distributions of PD and NSD being different.
\end{enumerate}

As the features are generally not distributed in a Gaussian fashion, we employ the non-parametric Kruskal-Wallis~\cite{kruskal1952use} ANOVA test, using the Dwass-Steel-Critchtlow-Flinger method to control for pairwise comparisons~\cite{DCSF}. Two groups are considered sampled from different distributions if $p<0.05$, with $p<0.05$ denoted with by a cross\textsuperscript{†} and $p<0.01$ denoted by a star\textsuperscript{*}. We may be interested in a result that the two distributions are the same; however, by the assumptions underlying $p$-value hypothesis testing, we can only conclude that that evidence in insufficent to show the distributions to be different~\cite{fisher1937design}. Bayesian tests such as the Bayes factor~\cite{bayesianttests} can quantify evidence for the   
...
\begin{figure}[!htbp]
	...
	\caption{(a) DFA is lower in PD vs control (Males $BF=10^5$, Females $BF=10^{15}$) and equal in PD vs NSD (Males $BF=5.9$, Females $BF=4.8$). (b) RPDE is lower in PD vs control (Males $BF=10^6$, Females $BF=25$) and equal in PD vs control (Male $BF=5.6$, Female $BF=5.2$)}
...
\begin{figure}[h]
	...
	\caption{(a) The fractal dimension is higher in PD vs control (Males $BF=500$, Females $BF=10^7$) and equal in PD vs NSD (Males $BF=5.5$, Females $BF=4.6$). (b) The sample entropy is highre in PD vs control (Males $BF=10^{10}$, Females $BF=10^7$) PLSFIXXXXXXX}
\end{figure}
