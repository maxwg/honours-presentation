...
Chapter \ref{bgchapter} summarises Parkinson's Disease and relevant prior work in the field of feature engineering and machine learning. The remainder of the thesis is written with the assumption that the reader understands this background, enabling it to be very concise. 
...
Unless there is evidence that `\textit{micro-symptoms}' are present in all people with PD, feature engineering in each of these areas are equally as important. Section \ref{speechbio} explores some of the biological causes of these symptoms and we will investigate the existence of  micro-symptoms in section X.X
...
\begin{table}[h]
\caption{Cross validation results of optimal SVM from random search using Tsanas' 10 feature ReliefF subset (fig \ref{tsanasresults}). Presented as mean $\pm$ stdev.}
...
\begin{table}[h]
	\caption{Cross validation results of optimal SVM using all speech features presented in Tsanas~et~al. (2012)~\cite{tsanas2012novel}. Equal split decisively outperforms \ref{tsanasfsresultsrelieff} with a Bayes factor of $10^{17}$.}
...
We evaluate both the features in Arora and the DREAM challenge baseline on the mPower accelerometer data. The DREAM baseline is expected to outperform Arora superset of Arora feature set. The DREAM baseline was calculated on both the butterworth filtered and original resting data, whereas no preprocessing was applied on the Arora data. The DREAM features are assumedly calculated as an extension of the original code in Arora et al.~\cite{arora2014high} whereas the Arora feature set are calculated with our implementation. The models were evaluated on a 50PD-50C split of the data to better convey performance.

\begin{table}[h]
	\caption{Cross validation results of optimal SVM on the mPower accelerometer data. Bayes factor of 1.2 --- insufficient evidence to conclude the DREAM set outperforms Arora.}
	...
	\begin{tabular}[t]{c c c}
		...
		\begin{tabular}{lcl}\hhline{~|*{2}{-}}
			...
			\multicolumn{2}{l}{Sensitivity (TP)}                                                                         & 60.0$\pm$6.4\%                                              \\
			\multicolumn{2}{l}{Specificity (TN)}                                                                         & 65.0$\pm$5.9\%                                             
		\end{tabular}
		& &
		\begin{tabular}{lcl}\hhline{~|*{2}{-}}
			\multicolumn{1}{l|}{}                                 & \multicolumn{1}{l|}{\cellcolor[HTML]{EFEFEF}Pred PD} & \multicolumn{1}{l|}{\cellcolor[HTML]{EFEFEF}Pred C} \\ \hline
			...
			\multicolumn{2}{l}{Specificity (TN)}                                                                         & $65.0\pm5.7$\%                                             
		\end{tabular}
	\end{tabular}
\end{table}
...
The poor performance is not unexpected --- it is highly unlikely that simple statistical and entropy measures are sufficient to accurately distinguish PD from control. There is a significant amount of noise into the data from sources such as whether the phone was in a pocket or bag and general variance in human gait. This noise cannot be adjusted for with these simple measures. More advanced signal processing techniques which may be more resistant to the noise will be required.



\section{Dynamical Systems Features and Data Boosting}
\label{databoosting}
The features engineered in current PD literature are clearly insufficient to accurately perform a diagnosis --- especially given the natural variance of biological signals and the difficult to quantify nature of PD. EEG signal processing (ref \ref{eegsigproc}) is a field that faces similar issues, characterising a  
...
\begin{figure}[h]
	...
	\label{dfapperpde}
	\centering\includegraphics[width=1\linewidth]{dfapperpde.png}
\end{figure}
